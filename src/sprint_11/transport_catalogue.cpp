...
namespace transport_catalogue { 
	size_t StopPairHash::operator()(const std::pair<const Stop*, const Stop*>& stops) const { 
	...
	void TransportCatalogue::AddStop(std::string stop_name, geo::Coordinates coordinates) { 
		if (stopname_to_stop.count(stop_name) == 0) { 
			...
			stopname_to_stop[view_name] = &stops_.back(); 
		} 
	} 
	...
	void TransportCatalogue::SetStopToStopDistances(const std::string stop_name, const std::string other_stop_name, double distance) { 
		if (stop_to_stop_distances_.count({ stopname_to_stop[other_stop_name], stopname_to_stop[stop_name] }) == 0) { 
			stop_to_stop_distances_[{ stopname_to_stop[stop_name], stopname_to_stop[other_stop_name] }] = distance; 
			stop_to_stop_distances_[{ stopname_to_stop[other_stop_name], stopname_to_stop[stop_name] }] = distance; 
		} 
		else { 
			stop_to_stop_distances_[{ stopname_to_stop[stop_name], stopname_to_stop[other_stop_name] }] = distance; 
		} 
	...
	void TransportCatalogue::AddBus(std::string bus_name, std::vector<std::string>& vect_stops, bool is_circle) { 
		if (busname_to_bus.count(bus_name) == 0) { 
			...
			for (std::string stop : vect_stops) { 
				stops.push_back(stopname_to_stop.at(stop)); 
				stops_with_routes_.push_back(stopname_to_stop.at(stop)->coordinates); 
			} 
			...
			busname_to_bus[bus_name_view] = &buses_.back(); 
			for (std::string_view stop : vect_stops) { 
				std::string_view stop_name = stopname_to_stop[stop]->name; 
				stop_to_buses[stop_name].insert(buses_.back().name); 
			} 
		} 
	} 
	...
	transport_catalogue::BusInfo TransportCatalogue::GetBusInfo(std::string_view name) const { 
		if (busname_to_bus.count(name) > 0) { 
			BusInfo bus_info{}; 
			const transport_catalogue::Bus* bus = busname_to_bus.at(name); 
	...
	transport_catalogue::StopInfo TransportCatalogue::GetStopInfo(std::string_view name) const { 
		if (stopname_to_stop.count(name) > 0) { 
			...
			auto stop_buses = GetBusesNamesByStop(name); 
			if (!stop_buses.empty()) { 
	...
	std::vector<const Stop*> TransportCatalogue::GetBusRouteByName(std::string_view name) const { 
		if (busname_to_bus.count(name) > 0) { 
			return busname_to_bus.at(name)->stops; 
		} 
	...
	Stop TransportCatalogue::GetStopByName(const std::string_view stop_name) const { 
		if (stopname_to_stop.count(stop_name) > 0) { 
			return { stopname_to_stop.at(stop_name)->name, stopname_to_stop.at(stop_name)->coordinates }; 
		} 
		static Stop empty_stop{}; 
		return empty_stop; 
	} 
 
	std::set<std::string_view> TransportCatalogue::GetBusesNamesByStop(std::string_view stop_name) const { 
