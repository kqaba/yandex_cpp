...
namespace renderer { 
    ...
    svg::Color MapRenderer::GetColor() const { 
        ...
        if (possible_colors_for_route_lines_.at(current_color_).IsString()) { 
            ...
            g = static_cast<uint8_t>(possible_colors_for_route_lines_.at(current_color_).AsArray().at(1).AsInt());    //сделать отдельный метод в приватном поле, Optional? GetCurrentColor; 
